...
Al confirmar el último paso, el receptor escribe, hace flush, verifica la fila definitiva y devuelve la confirmación. La pantalla muestra «Estamos registrando tus datos» durante esa solicitud y agradece solo al confirmar el guardado. Ante error vuelve a mostrar la ficha intacta para reintentar. La generación del Excel y el envío del correo no forman parte de la respuesta al navegador.

La propia fila confirmada es la cola durable: el activador de `processAltaNotifications`, programado cada minuto, toma las altas pendientes aunque el cliente ya haya cerrado la página. Google puede demorar la ejecución del activador; no se garantiza un plazo exacto de entrega. Navegar entre pasos no guarda ni envía correos. Un fallo del mail no revierte el alta ni produce un falso error de persistencia. El bloqueo serializa los procesadores y protege la escritura y la deduplicación. `setupAltaNotifications` prepara la pestaña «Avisos de alta» y crea un único activador si no existe. Verificar que ese activador esté activo antes de publicar esta versión. Usa MailApp con permiso de envío solamente, más el permiso para administrar el activador. El remitente es la cuenta que ejecuta el activador.
